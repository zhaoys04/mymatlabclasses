function [R T] = getRT(MLS, wavelength, nkx)
% [R T] = getRT(wavelength, nkx):
% 	wavelength: wavelength of incident light. Unit should be consist with other length
% 	nkx: relative/normalized transverse wave vector. nkx = kx/k0
%	return value, R and T are structures with two fields, representing p and s polarized incident wave.
	Ms = diag([1,1]);
	Mp = diag([1,1]);
...
%		display(kz);
	...
	R.Rs = -Ms(2,1)/Ms(2,2);
	R.Rp = -Mp(2,1)/Mp(2,2);
	T.Ts = Ms(1,1)+Ms(1,2)*R.Rs;
	T.Tp = Mp(1,1)+Mp(1,2)*R.Rp;
